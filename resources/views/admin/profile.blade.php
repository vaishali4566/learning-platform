...
<div class="relative min-h-screen flex items-center justify-center px-6 py-10 overflow-hidden 
    ...
    <div class="relative z-10 w-full max-w-5xl bg-white/10 backdrop-blur-2xl rounded-md shadow-[0_0_40px_rgba(0,194,255,0.1)] border border-white/10 p-8 md:p-10 flex flex-col md:flex-row gap-10
        ...
        <div class="md:w-1/3 flex flex-col items-center text-center space-y-4 border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-6">
            ...
            <div>
                <h2 class="text-xl font-semibold text-[#E6EDF7]">{{ $admin->name }}</h2>
                <p class="text-[#9CA3B0] text-sm">{{ $admin->email }}</p>
                <p class="text-[#9CA3B0] text-xs mt-1 italic tracking-wide">Administrator</p>
            </div>
        </div>
        ...
        <div class="md:w-2/3">
            <h3 class="text-lg md:text-xl font-semibold text-[#E6EDF7] mb-5 flex items-center gap-2 border-b border-white/10 pb-2">
                <span class="text-[#00C2FF]">🛠️</span> Admin Settings
            </h3>
            ...
            <form action="{{ route('admin.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="bio" rows="3"
                              class="w-full bg-[#1C2541]/70 text-[#E6EDF7] border border-[#00C2FF]/20 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00C2FF]/70 resize-none text-sm transition-all duration-300 placeholder-gray-400">{{ old('bio', $admin->bio) }}</textarea>
                </div>
