...
@section('content')
<div class="relative min-h-full flex items-center justify-center px-6 py-10 overflow-hidden 
    ...
    <!-- Glassmorphic Trainer Profile Container -->
    <div class="relative z-10 w-full max-w-5xl bg-white/10 backdrop-blur-2xl rounded-md shadow-[0_0_40px_rgba(0,194,255,0.1)] border border-white/10 p-8 md:p-10 flex flex-col md:flex-row gap-10
        ...
        <div class="md:w-1/3 flex flex-col items-center text-center space-y-4 border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-6">
            ...
            <div>
                <h2 class="text-xl font-semibold text-[#E6EDF7]">{{ $trainer->name }}</h2>
                <p class="text-[#9CA3B0] text-sm">{{ $trainer->email }}</p>
                <p class="text-[#9CA3B0] text-xs mt-1 italic tracking-wide">{{ $trainer->country ?? '—' }}</p>
            </div>
        </div>
        ...
        <div class="md:w-2/3">
            ...
            <form action="{{ route('trainer.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-3 pt-3">
                    ...
                    <!-- 🔹 Logout Button -->
                    <form action="{{ route('trainer.logout') }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"
